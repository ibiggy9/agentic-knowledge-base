"""
Claude MCP Client (deprecated). Prefer the Gemini client for new development.

No API keys are stored in this repository. Configure ANTHROPIC_API_KEY at runtime if using.
"""
...
class MCPClient:
    ...
    async def connect_to_server(self, server_script_path: str) -> None:
        ...
        print(
            ...
            [tool.name for tool in self.tools_cache],
        )
        ...
        while True:
            try:
                ...
                if iteration_count % 1000 == 0 or current_time - last_log_time > 5:
                    logger.info(
                        f"Worker iteration {iteration_count}, queue size: {self.request_queue.qsize()}, idle_count: {idle_count}"
                    )
                ...
                try:
                    result = await asyncio.wait_for(
                        self.session.call_tool(tool_name, tool_args), timeout=5.0
                    )
        ...
        **kwargs: Any,
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_exception = e
                logger.warning(
                    "Call failed on attempt %d/%d: %s", attempt + 1, max_retries, str(e)
                )
    ...
    async def call_claude_with_fallback(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "Model fallback call failed: %s", str(model_fallback_error)
                )
            ...
            if tools:
                try:
                    ...
                    logger.error(
                        "Fallback Claude call also failed: %s", str(fallback_error)
                    )
    ...
    async def _handle_help_request(self, query: str) -> str:
        ...
        self.token_tracker.add_usage(
            response.usage.input_tokens, response.usage.output_tokens, "Help Request"
        )
    ...
    async def strategic_analysis(self, query: str) -> str:
        ...
        for i, step in enumerate(steps):
            ...
            tool_calls = [
                content
                for content in execution_response.content
    ...
    def _parse_strategy_steps(self, strategy_text: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if match:
                ...
                steps.append({"number": number, "description": description})
    ...
    def generate_test_args(
        self, input_schema: Union[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        if not input_schema:
            return {}
    ...
    async def diagnose_server(self) -> Dict[str, Any]:
        diag_results = {
            ...
            "tools_available": 0,
            "connected_since": (
                self.connected_at if hasattr(self, "connected_at") else None
            ),
                ...
                if tools_response.tools:
                    ...
                    try:
                        await asyncio.wait_for(
                            self.session.call_tool(test_tool.name, test_args),
                            timeout=10.0,
                        )

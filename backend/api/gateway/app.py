...
from .client_claude import MCPClient as ClaudeClient
...
async def load_mcp_client(server_type: str):
    try:
        ...
        raise Exception(f"Error initializing MCPClient: {str(e)}\nFull traceback: {full_traceback}")
...
async def initialize_session(request: InitRequest):
    try:
        ...
        logger.info(f"Initializing {request.server_type} server with session {session_id}")
        ...
        try:
            ...
            if "anyio" in str(e).lower() or "subscriptable" in str(e).lower():
                logger.error("This appears to be an anyio compatibility issue. Check your package versions.")
                return {
                    "status": "error",
...
async def get_server_types():
    return {
        "server_types": [
            {"id": "rfx", "name": "RFX Database", "description": "Analyze RFX database with SQL queries"},
            {"id": "samsara", "name": "Samsara Analysis", "description": "Analyze Samsara documents from Google Drive"},
            {"id": "raw_rfx", "name": "Raw RFX Data", "description": "Analyze individual RFX sales calls and RFP requests"}
        ]
    }
...
async def stream_query(session_id: str = Query(...), query: str = Query(...)):
    async def event_generator():
        try:
                ...
                try:
                    def progress_callback(message, details=None):
                        ...
                        asyncio.create_task(progress_queue.put({
                            ...
                            'metrics': modified_details.get('metrics') if modified_details else None,
                            'timestamp': time.time()
                        }))
            ...
            while not (processing_complete and progress_queue.empty() and final_response_sent):
                try:
                    result = await asyncio.wait_for(progress_queue.get(), timeout=timeout_seconds)
                    consecutive_timeouts = 0
                    ...
                    if consecutive_timeouts >= max_consecutive_timeouts and processing_complete:
                        logger.warning(f"Stream ended after {consecutive_timeouts} consecutive timeouts")

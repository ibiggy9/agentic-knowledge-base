...
class TokenTracker:
    ...
    def add_usage(self, input_tokens: int, output_tokens: int, description: str = "API call"):
...
class MCPClient:
                            ...
                            if item.get("type") == "text":
                                ...
                                for content_item in tool_content:
                                    if isinstance(content_item, dict) and content_item.get("type") == "text":
                                        result_parts.append(content_item.get("text", ""))
                                
                                if result_parts:
                                    formatted_result = f"Tool result from {tool_name}:\n" + "\n".join(result_parts)
    ...
    async def connect_to_server(self, server_script_path: str):
        ...
        print("\n-----------------------------------------")
        print("Welcome to the Gemini MCP Client")
        print("-----------------------------------------")
    ...
    async def call_gemini_with_fallback(self, messages, system=None, tools=None, max_tokens=5000, temperature=None, model=None):
        try:
                ...
                for tool in tools:
                    tool_config = {
                        "function_declarations": [{
                            ...
                            "parameters": self._clean_schema_for_gemini(tool.get("input_schema", {}))
                        }]
                    }
            ...
            if hasattr(response, 'usage_metadata'):
                ...
                output_tokens = len(self.extract_text_from_response(response).split()) * 1.3
            
            self.token_tracker.add_usage(int(input_tokens), int(output_tokens), "Primary API Call")
            ...
            try:
                ...
                self.token_tracker.add_usage(int(input_tokens), int(output_tokens), "Fallback API Call")
                ...
                if tools:
                    try:
                        ...
                        self.token_tracker.add_usage(int(input_tokens), int(output_tokens), "Fallback without tools")
                        
                        response._no_tools_fallback = True
                        return response
                    
                    except Exception as fallback_error:
                        logger.error(f"Fallback without tools failed: {str(fallback_error)}")
    ...
    def _clean_schema_for_gemini(self, schema):
        ...
        for key, value in schema.items():
            if key == "default":
                ...
                for prop_name, prop_schema in value.items():
                    clean_properties[prop_name] = self._clean_schema_for_gemini(prop_schema)
        ...
        if clean_schema.get("type") == "array" and "items" in clean_schema:
            if isinstance(clean_schema["items"], dict) and "type" not in clean_schema["items"]:
    ...
    async def process_tool_calls(self, response):
        ...
        try:
                ...
                for function_call in function_calls:
                    ...
                    if hasattr(function_call, 'args'):
                        if isinstance(function_call.args, dict):
                            ...
                            logger.warning(f"Unexpected args format: {type(function_call.args)}")
            ...
            for req in pending_requests:
                ...
                if request_id in self.response_map:
                    ...
                    if response["success"]:
                        result_content = []
                        if hasattr(response["result"], "content") and response["result"].content:
                            for item in response["result"].content:
                                if hasattr(item, 'type') and hasattr(item, 'text'):
                                    result_content.append({"type": item.type, "text": item.text})
                        ...
                        results.append({
                            ...
                            "metadata": {
                                "result_type": "success",
                                "size": len(str(response["result"].content)) if hasattr(response["result"], "content") else 0
                            }
                        })
                    else:
                        results.append({
                ...
                if tool_name:
                    if "query" in tool_name.lower() or "search" in tool_name.lower():
                        ...
                        try:
                            ...
                            if file_count > 0:
                                self.operation_metrics["documents_processed"] += file_count
                            else:
        ...
        self._report_progress("Updated operation metrics", {
    ...
    async def process_query(self, query: str) -> str:
        ...
        classification = self.extract_text_from_response(initial_response).strip().upper()
    ...
    async def strategic_analysis(self, query: str) -> str:
        ...
        tool_descriptions = "\n".join([f"- {tool['name']}: {tool['description'] or 'No description'}" for tool in available_tools])
        
        print("\n" + "="*80)
        print(" PHASE 1: QUESTION ANALYSIS ".center(80, "="))
        print("="*80)
        
        self._report_progress("""IMPORTANT FORMATTING INSTRUCTIONS:
        ...
        self._report_progress("Phase 3: Executing analysis plan with progressive insight development...")
        ...
        for i, step in enumerate(steps):
            ...
            for result in step_results:
                ...
                if result['success']:
                    content = result["result"].content if hasattr(result["result"], "content") else []
                    content_text = ""
                    for item in content:
                        if hasattr(item, 'text'):
                            content_text += item.text
                    print(f"Success: {result['success']}")
                    print(f"Content: {content_text[:500]}..." if len(content_text) > 500 else f"Content: {content_text}")
                else:
                    print(f"Error: {result['error']}")
                print("-"*40)
            
            self._report_progress(f"Successfully completed step {i+1}", {
                "step": i+1,
                "total": total_steps,
                "status": "completed",
                "tool": step_results[0]["tool"] if step_results and len(step_results) > 0 else "unknown",
            })
            ...
            intermediate_insights.append(f"\n--- INSIGHTS FROM STEP {i+1} ---\n{step_insights}")
            
            execution_context += f"\n\n--- STEP {i+1} RESULTS ---\n{result_text}\n\n--- STEP {i+1} INSIGHTS ---\n{step_insights}"
            
            self._report_progress(f"Step {i+1}/{total_steps} processed data", {
                "metrics": self.operation_metrics,
                "step": i+1,
                "status": "in-progress"
            })
    ...
    def _parse_strategy_steps(self, strategy_text: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            elif "**Information Obtained:**" in line or "- Information Obtained:" in line:
                in_parameters = False
                in_information = True
                in_contribution = False
    ...
    async def manage_context(self):
        ...
        for i, msg in enumerate(self.conversation_history):
            ...
            if isinstance(content, list) and any(item.get("type") == "tool_result" for item in content if isinstance(item, dict)):
                tool_call_indices.append(i)
        
        recent_tool_calls = [i for i in tool_call_indices if i >= len(self.conversation_history) - keep_count * 2]
        ...
        seen = set()
        important_context = [x for x in important_context if not (x in seen or seen.add(x))]
    ...
    def _sanitize_tool_args(self, tool_name, tool_args):
        ...
        if tool_name in ["search_drive_files", "list_drive_files", "list_drive_folders"]:
        ...
        if tool_name == "list_drive_folders":
            if "parent_folder_id" in sanitized_args and sanitized_args["parent_folder_id"] is None:

...
export const MarkdownText = ({ content }) => (
  <ReactMarkdown
    components={{
      h1: ({ node, ...props }) => (
        <h1 className="text-xl font-bold border-b border-[#40444B] pb-1 mb-3" {...props} />
      ),
      ...
      pre: ({ node, ...props }) => (
        <pre className="bg-[#2F3136] rounded-lg p-4 my-2 overflow-x-auto" {...props} />
      ),
      code: ({ node, inline, className, children, ...props }) => {
        if (inline) {
          return (
            <code className="bg-[#2F3136] rounded px-1 py-0.5 text-sm" {...props}>
              {children}
            </code>
          );
        }

...
import { 
  ...
  handleTyping, 
  ContentRenderer 
} from "@/Components/ChatBots/ChatUtils";
...
export default function Chat() {
  ...
  const generateRandomSuggestions = (currentServerType) => {
    const inputList = 
      currentServerType === "rfx" ? userInputsRfx : 
      currentServerType === "raw_rfx" ? userInputsRawRfx : 
      userInputsSamsara;
  ...
  async function initSession(serverType) {
    ...
    // DOESNT WORK ATM - ADJUSTING FROM CONNECTION_LOADER DEPRECATION
    setProgressIndicator(serverType === "rfx" ? "Connecting to RFX knowledge base..." : "Connecting to Samsara knowledge base...");
  ...
  async function fetchServerTypes() {
    try {
      const response = await fetch("http://localhost:8000/api/server-types");
      const data = await response.json();
  ...
  const handleResetChat = useCallback(async (newServerType = null) => {
    ...
    if (sessionIdRef.current) {
      try {
        await fetch(`http://localhost:8000/api/session/${sessionIdRef.current}`, {
          method: "DELETE",
        });
      } catch (error) {
  ...
  const handleServerTypeChange = useCallback(async (type) => {
    if (serverType === type) return;
    
    setIsConnecting(true);
    setProgressIndicator(type === "rfx" ? "Connecting to RFX knowledge base..." : "Connecting to Samsara knowledge base...");
  ...
  function handleSubmit(e) {
    ...
    try {
      ...
      const url = `http://localhost:8000/api/query-stream?session_id=${sessionId}&query=${encodedQuery}`;
      ...
      eventSource.onmessage = (event) => {
        ...
        console.log("SSE update (FULL DATA):", data);
        
        if (data.details && data.details.status === "completed") {
          console.log("STEP COMPLETION DATA RECEIVED:", data.details);
        }
        ...
        if (data.type === 'final') {
          ...
          setTimeout(() => {
            setMessages(prev => 
              prev.map((msg, i) => 
                i === prev.length - 1 ? { ...msg, hasCompleted: true } : msg
              )
            );
            setIsTyping(false);
          }, 100);
  ...
  async function fallbackToRegularApi(userMessage) {
    let progressPhases = serverType === "rfx" 
      ? ["Analyzing data...", "Running queries...", "Analyzing patterns...", "Generating insights..."]
      : ["Processing documents...", "Extracting information...", "Analyzing content...", "Generating insights..."];
                  ...
                  <div className="flex-1 relative overflow-hidden pr-24">
                    <textarea
                      ...
                      placeholder={isConnected 
                        ? (serverType === "rfx" ? "Analyze RFX data..." : "Ask about Samsara documents...")
